...
let next_position(pt, dir : t_position * int) : t_position =
  ...
  let indexes : int matrix = [| [|1; 0|]; [|0; 1|]; [|-1; 0|]; [|0; -1|] |] in
...
let insert_rand_ship(nb_pos, m, l, prm : int * t_matrix * t_ship_remain * t_param) : unit = 
  ...
  do
    ...
    let direction : int = rand_int(0, 3) in
...
@return deux joueurs
*)
let init_play(nm1, hum1, nm2, hum2, id, prm : string * bool * string * bool * string * t_param) : t_play = 
  { pl1 = init_player(nm1, hum1, prm); pl2 = init_player(nm2, hum2, prm); prm = prm; nextPlayer = ref NULL; gameId = id }
;;
...
*)
let rec choose_one_shoot(l, m, opp_ships, prm : t_pos_list * t_matrix* t_ship_remain_opp_list * t_param) : t_position =
  (* let rand_pos : t_position = rand_position(prm) in *)
  let rand_pos : t_position = get_ai_shoot(m, opp_ships ,prm) in
  if not(position_in_list(rand_pos, l)) && is_empty(rand_pos.cx, rand_pos.cy, m)
    then rand_pos
  else choose_one_shoot(l, m, opp_ships, prm)
;;
...
*)
let choose_shoots(nb_shoots, opp_sh, m, prm : int * t_ship_remain_opp_list * t_matrix * t_param) : t_pos_list =
  let rec choose_shoots_aux(nb, l, m, prm : int * t_pos_list * t_matrix * t_param) : t_pos_list = 
    if nb = 0 then l
    else 
      choose_shoots_aux(nb - 1, add_lst(l, choose_one_shoot(l, m, opp_sh, prm)), m, prm);
  in
...
let play_one_player_computer(player_1, player_2, prm : t_player * t_player * t_param) : int =
  all_shoots(choose_shoots(1, player_1.opp_remain ,player_1.opp_mat ,prm), player_1, player_2, prm)
;;
...
(* let play_sim(nm_1, hum_1, nm_2, hum_2 : string * bool * string * bool) : int =
  rand_init();
  let prm : t_param = init_param() and who : int ref = ref 1 
  and the_end : bool ref = ref false and nb_failed : int ref = ref 0
  in
  let pl : t_play = init_play(nm_1, hum_1, nm_2, hum_2, "aaa", prm) in
  let rounds : int ref = ref 0 in
  ...
  print_string("Game took: " ^ string_of_int(!rounds / 2) ^ "\n");
  !rounds / 2
;; *)


let play(nm_1, hum_1, nm_2, hum_2 : string * bool * string * bool) : int =
  rand_init();
  let prm : t_param = init_param() and who : int ref = ref 1 
  and the_end : bool ref = ref false and nb_failed : int ref = ref 0
  in
...
play("sah", false, "sah", false);;

(* let play_test(nm_1, hum_1, nm_2, hum_2 : string * bool * string * bool) : unit =
  rand_init();
  let prm : t_param = init_param() and who : int ref = ref 1 
  and nb_failed : int ref = ref 0
  in
  let pl : t_play = init_play(nm_1, hum_1, nm_2, hum_2, "aaa", prm) in
  
  play_one_player(pl.pl1, pl.pl2, prm) ;
   play_one_player(pl.pl2, pl.pl1, prm) ;
  (* print_string("Content-type: application/json\n\n"); *)
  (* Yojson.Safe.pretty_to_string(t_player_to_yojson pl.pl1); *)
  let s : string = Yojson.Safe.to_string (t_player_to_yojson pl.pl1) in
  let oc = open_out "file.txt" in
  Printf.fprintf oc "%s\n" s;
  close_out oc;
  print_string(s);
  (* Printf.fprintf (open_out "file.txt") s *)
;; *)
...
(* let avg : int ref = ref 0;;

for i=0 to 49
do
  avg := !avg + play_sim("sah1", false, "sah2", false);
done;

print_string("Took: "^string_of_int(!avg  / 49)^" rounds\n");; *)


(* normal avg: 59 ? *)
(* avg with skrewing: 59 *)


let save_game_info(id, data : string * t_play) : unit = 
  let name : string = "games/" ^ id ^ ".json" in
  let s : string = Yojson.Safe.to_string(t_play_to_yojson(data)) in
...
let get_game_info(id : string) : t_play = 
  let name : string = "games/" ^ id ^ ".json" in

  (* pas la moindre idée de pourqoi faut faire comme ca, merci google *)
  let x =(
    let x_result = t_play_of_yojson (Yojson.Safe.from_file name ) in
    (function
      | Result.Ok y -> y
    ) x_result)  in
  x
...
let print_game_info(player, prm, gameId : t_player * t_param * string) : unit =
  print_string("{\"player1\":"^Yojson.Safe.to_string(t_player_to_yojson(player))^",\"prm\":" ^ Yojson.Safe.to_string(t_param_to_yojson(prm))^ ", \"gameId\": \"" ^ gameId ^ "\"}")
;;